...
app.post('/submit-payment-proof', async (req, res) => {
  ...
  try {
      ...
      // Insert the record into payment_requests table with the SIGNED URL
      const { error: dbError } = await supabase
          ...
          .insert({
              ...
              plan: plan,
              screenshot_url: signedUrl, // Store the signed URL
              status: 'pending'
          });
...
app.post('/generate-tts', async (req, res) => {
  ...
  try {
    const modelName = 'gemini-2.5-flash-native-audio-preview-12-2025'; // Ensure to use the correct live audio model for consistency
