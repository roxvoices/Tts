...
app.use(cors());
app.use(bodyParser.json({ limit: '50mb' })); // Increase limit for base64 audio
...
function createWavHeader(dataSize: number) {
  ...
  writeString(8, 'WAVE');
  view.setUint32(8, 36 + dataSize, true);
  writeString(8, 'WAVE');
  writeString(12, 'fmt ');
...
// Map frontend expressions to system instructions (as Gemini TTS doesn't have direct expression control)
const mapExpressionToSystemInstruction = (expression) => {
  switch (expression) {
    case 'Professional': return 'Speak in a clear, formal, and articulate manner.';
    case 'Cheerful': return 'Speak in a happy, bright, and optimistic tone.';
    case 'Somber': return 'Speak in a serious, melancholic, and solemn tone.';
    case 'Whispering': return 'Speak in a very soft, hushed, and secretive manner.';
    case 'Authoritative': return 'Speak in a firm, commanding, and confident tone.';
    case 'Excited': return 'Speak with high energy, enthusiasm, and quick pacing.';
    case 'Natural':
    default: return 'Speak in a natural, conversational, and balanced tone.';
  }
};
...
const getUserData = (userId, initialSubscription = 'free') => {
  if (!database.users[userId]) {
    const now = new Date();
    const todayCAT = new Date(now.toLocaleString('en-US', { timeZone: 'Africa/Lusaka' }));
    todayCAT.setHours(0, 0, 0, 0); // Set to 00:00:00.000 CAT
    database.users[userId] = {
      dailyCharsUsed: 0,
      dailyLimitResetTime: todayCAT.toISOString(), // Start of today CAT
  ...
  if (nowCAT.toDateString() !== resetDateCAT.toDateString()) {
...
app.get('/user-profile', (req, res) => {
  ...
  const userData = getUserData(userId); // This will create/update user data including daily reset
  res.json(userData);
});
...
app.post('/generate-tts', async (req, res) => {
  ...
  try {
    const modelName = 'gemini-2.5-flash-native-audio-preview-12-2025'; // Text-to-speech model
    const systemInstruction = mapExpressionToSystemInstruction(settings.expression);
      ...
      config: {
        ...
        speechConfig: {
          voiceConfig: {
            ...
            // Gemini TTS does not directly support pitch/speed parameters via prebuiltVoiceConfig
            // The model generally adapts to systemInstruction and the default voice properties.
            // If explicit pitch/speed control is needed, it typically requires a custom voice
            // or a model that exposes these parameters. For now, systemInstruction guides tone.
            // For a frontend demo, the sliders control client-side playback pitch/speed, not API generation.
            // If the backend needs to affect pitch/speed of generation, it would require a model/API that supports it.
            // For now, we omit passing settings.pitch and settings.speed to the API call directly.
          },
        },
        systemInstruction: systemInstruction,
        // The Gemini TTS API does not expose `pitch` and `speed` directly in `speechConfig` for `prebuiltVoiceConfig`.
        // These are often handled by the specific voice model's underlying characteristics or
        // by more advanced custom voice configuration. We'll rely on systemInstruction to guide the tone.
        // For a frontend demo, the sliders control client-side playback pitch/speed, not API generation.
        // If the backend needs to affect pitch/speed of generation, it would require a model/API that supports it.
        // For now, we omit passing settings.pitch and settings.speed to the API call directly.
      },
...
app.delete('/delete-tts/:projectId', (req, res) => {
  ...
  userData.dailyCharsUsed = Math.max(0, userData.dailyCharsUsed - textLength); // Deduct characters
