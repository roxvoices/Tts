...
const API_KEY = process.env.GEMINI_API_KEY; // Aligned with @google/genai guidelines and common practice
if (!API_KEY) {
...
// Helper to create WAV header (corrected)
...
app.post('/generate-tts', async (req, res) => {
  ...
  try {
    ...
    // Ensure systemInstruction is applied
    const systemInstruction = mapExpressionToSystemInstruction(settings.expression);
    const finalPrompt = `Read the following transcript exactly: ${text}`; // Model is designed to speak, not converse
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        systemInstruction: systemInstruction, // Apply system instruction for tone
      },
    });
