...
// New helper function to map expressions to adverbial phrases for the prompt
const mapExpressionToPromptModifier = (expression) => {
  switch (expression) {
    case 'Professional': return 'professionally';
    case 'Cheerful': return 'cheerfully';
    case 'Somber': return 'somberly';
    case 'Whispering': return 'in a whispering tone'; // Use a phrase for whispering
    case 'Authoritative': return 'authoritatively';
    case 'Excited': return 'excitedly';
    case 'Natural':
    default: return ''; // No prefix for natural tone
  }
};
...
app.post('/generate-tts', async (req, res) => {
  ...
  try {
    ...
    // Embed expression directly into the prompt text using adverbial phrases
    const expressionModifier = mapExpressionToPromptModifier(settings.expression);
    let finalPrompt = text;
    if (expressionModifier) {
      // For 'Whispering', use a slightly different phrasing for clarity to the model
      if (settings.expression === 'Whispering') {
        finalPrompt = `Speak ${expressionModifier}: ${text}`;
      } else {
        finalPrompt = `Say ${expressionModifier}: ${text}`;
      }
    }
